...
{
    // Multi-objective optimization: combine CN (CND score) and ND (max component)
    // Both metrics are normalized to [0,1] range
    
    double orig_node_num = (double) graph->num_nodes;
    double weight_ratio = graph->nodes_weight[a] / graph->total_nodes_weight;
    
    // ND metric: normalized max connected component size
    double nd_normalized = getMaxConnectedNodesNum() / orig_node_num;
    
    // CN metric: normalized CND score (component number density)
    double cnd_score = getRemainingCNDScore();
    double max_cnd = orig_node_num * (orig_node_num - 1) / 2.0;
    double cn_normalized = cnd_score / max_cnd;
    
    // Linear combination with equal weights (can be tuned: alpha for CN, (1-alpha) for ND)
    double alpha = 0.5;  // 0.5 = equal weight, 0.0 = pure ND, 1.0 = pure CN
    double combined_metric = alpha * cn_normalized + (1.0 - alpha) * nd_normalized;
    
    // Reward = -combined_metric * weight_ratio (negative because we want to minimize)
    return -combined_metric * weight_ratio;
}
...
double MvcEnv::getRemainingCNDScore()
{
    ...
    for(std::set<int>::iterator it = lccIDs.begin(); it != lccIDs.end(); it++)
    {
        ...
        CCDScore += (double) num_nodes * (num_nodes - 1) / 2.0;
    }
