...
//        printf (" reInsertCount:%d\n", leftOutput.size());
        ...
        for (int eachNode : leftOutput)
        {
            ...
            batchList.push_back(make_pair((double)decreaseValue*graph->nodes_weight[eachNode], eachNode));
        }
...
double Utils::getRobustness(std::shared_ptr<Graph> graph, std::vector<int> solution, double alpha)
{
    assert(graph);
    MaxWccSzList.clear();
    std::vector<std::vector<int>> backupCompletedAdjListGraph = graph->adj_list;
    ...
    double totalND = 0.0;  // ND累积损伤：最大连通分量
    double totalCN = 0.0;  // CN累积损伤：连通分量数量密度
    ...
    for (int i = (int)solution.size()-1;i>=0;i=i-1)
    {
        ...
        // 计算ND损伤（最大连通分量）
        double nd_value = (double)disjoint_Set.maxRankCount / (double)graph->num_nodes;
        totalND += nd_value * solution_weights;
        
        // 计算CN损伤（连通分量数量密度）
        std::set<int> component_ids;
        for(int j = 0; j < graph->num_nodes; j++){
            if(backupAllVex[j]){
                component_ids.insert(disjoint_Set.unionSet[j]);
            }
        }
        double cn_score = 0.0;
        for(std::set<int>::iterator it = component_ids.begin(); it != component_ids.end(); it++)
        {
            double num_nodes = (double)disjoint_Set.getRank(*it);
            cn_score += num_nodes * (num_nodes - 1) / 2.0;
        }
        double max_cn = (double)graph->num_nodes * (graph->num_nodes - 1) / 2.0;
        double cn_value = (max_cn > 0) ? (cn_score / max_cn) : 0.0;
        totalCN += cn_value * solution_weights;
        
        MaxWccSzList.push_back(nd_value);
    }

    std::reverse(MaxWccSzList.begin(), MaxWccSzList.end());

    // 多目标评估：alpha*CN + (1-alpha)*ND
    double totalScore = alpha * totalCN + (1.0 - alpha) * totalND;
    return (double)totalScore / graph->total_nodes_weight;
}
