...
#################################################New code for FINDER#####################################
    ...
    def findModel(self):
        """
        找到验证集上表现最好的模型
        对于多目标优化(CND_cost)，选择VC值最小的模型
        """
        save_dir = './models/Model_%s'%(self.g_type)
        VCFile = '%s/ModelVC_%d_%d.csv'%(save_dir, NUM_MIN, NUM_MAX)
        
        if not os.path.exists(VCFile):
            print(f"⚠️ 警告: 找不到性能记录文件 {VCFile}")
            print("   使用最新保存的模型")
            # 查找最新的checkpoint
            import glob
            ckpt_files = glob.glob(f'{save_dir}/nrange_{NUM_MIN}_{NUM_MAX}_iter_*.ckpt.index')
            if not ckpt_files:
                raise FileNotFoundError(f"未找到任何模型文件在 {save_dir}")
            latest_ckpt = max(ckpt_files, key=os.path.getctime)
            best_model = latest_ckpt.replace('.index', '')
        ...
        # 跳过前33次迭代（预热阶段）
        start_loc = 33
        if len(vc_list) <= start_loc:
            print(f"⚠️ 警告: 训练数据不足 (只有{len(vc_list)}条记录)")
            best_model_iter = (len(vc_list) - 1) * 300
        else:
            # 找到最小VC值的位置
            min_vc = start_loc + np.argmin(vc_list[start_loc:])
            best_model_iter = 300 * min_vc
        
        best_model = '%s/nrange_%d_%d_iter_%d.ckpt' % (save_dir, NUM_MIN, NUM_MAX, best_model_iter)
        
        if not os.path.exists(best_model + '.index'):
            print(f"⚠️ 警告: 最优模型文件不存在 {best_model}")
            print("   尝试查找最接近的模型...")
            import glob
            ckpt_files = glob.glob(f'{save_dir}/nrange_{NUM_MIN}_{NUM_MAX}_iter_*.ckpt.index')
            if ckpt_files:
                # 找到迭代次数最接近的模型
                iterations = [int(f.split('_iter_')[1].split('.ckpt')[0]) for f in ckpt_files]
                closest_iter = min(iterations, key=lambda x: abs(x - best_model_iter))
                best_model = '%s/nrange_%d_%d_iter_%d.ckpt' % (save_dir, NUM_MIN, NUM_MAX, closest_iter)
                print(f"   使用: {best_model}")
    ...
    def EvaluateSol(self, data_test, sol_file, strategyID=0, reInsertStep=20):
        ...
        Robustness = self.utils.getRobustness(g_inner, solution, alpha=0.5)
    ...
    def Test(self,int gid):
        ...
        Robustness = self.utils.getRobustness(g_list[0], solution, alpha=0.5)
        return Robustness
    ...
    def HXA(self, g, method):
        ...
        Robustness = self.utils.getRobustness(self.GenNetwork(g), solutions, alpha=0.5)
        return Robustness
    
    def HXAWithSol(self, g, method):
        """HXA方法，返回鲁棒性和攻击序列"""
        sol = []
        G = g.copy()
        while (nx.number_of_edges(G)>0):
