...
# 全局参数设置
# ==========================================
ROOT_GML_DIR = 'saved_graphs/degree_cost_weighted'  # 图文件目录（改为有权重的图）
OUT_ROOT = './attack_outputs/weighted_axis_test_nd_cost'  # 输出目录
...
print(f"✓ 使用ND-cost模型: iter={best_iter}")
...
def generate_attack_sequences(gml_path, graph_idx=0):
    ...
    for m in METHODS:
        if m == 'RF':
            ...
            rob, sol = dqn.HXAWithSol(G, m)  # 使用HXAWithSol方法返回两个值
            sequences[m] = sol
