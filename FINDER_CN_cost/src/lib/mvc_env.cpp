...
//    return -(double)(getMaxConnectedNodesNum()/graph->num_nodes)*(graph->nodes_weight[a]/graph->total_nodes_weight);
    // Use CND score (Component Number Density) like FINDER_CN for consistency
    double orig_node_num = (double) graph->num_nodes;
    return -(double)(getRemainingCNDScore()/(orig_node_num*orig_node_num*(orig_node_num-1)/2))*(graph->nodes_weight[a]/graph->total_nodes_weight);
}
