...
def simulate_attack(G, seq):
    ...
    PC使用CND Score方法（Component Number Density）：
    - CND = sum(n_i * (n_i - 1) / 2) for all connected components
    - 衡量所有连通分量的配对连通性总和
    """
    GG = G.copy()
    ...
    # 计算初始CND Score（连通分量密度分数）
    # 初始时整个图是一个连通分量：CND0 = N*(N-1)/2
    cnd0 = N0 * (N0 - 1) / 2.0
    ...
    for action in seq:
        ...
        if GG.number_of_nodes() == 0:
            ...
            for component in connected_components:
                n_i = len(component)
                curr_cnd += n_i * (n_i - 1) / 2.0
            
            # 归一化
            pc = curr_cnd / cnd0 if cnd0 > 1e-9 else 0.0
