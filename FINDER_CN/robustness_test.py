...
def main():
    ...
    # 图表格式
    # ==========================
    axes[0].set_title("Network Dismantling: GCC Size vs Removed Nodes Cost", fontsize=14, fontweight='bold')
    axes[0].set_xlabel("Fraction of removed nodes cost", fontsize=12)
    axes[0].set_ylabel("GCC Size (LCC节点数/总节点数)", fontsize=12)
    axes[0].set_xlim(0, 0.6)
    axes[0].set_ylim(0, 1.05)
    axes[0].legend(loc='upper right')
    axes[0].grid(True, alpha=0.3)

    axes[1].set_title("PC vs Fraction of Removed Nodes (log scale)", fontsize=14, fontweight='bold')
    axes[1].set_xlabel("Fraction of removed nodes q", fontsize=12)
    axes[1].set_ylabel("PC / PC₀", fontsize=12)
