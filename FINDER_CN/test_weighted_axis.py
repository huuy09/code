...
# 全局参数设置
# ==========================================
ROOT_GML_DIR = 'saved_graphs/uniform_cost/30-50'  # 图文件目录
OUT_ROOT = './attack_outputs/weighted_axis_test'  # 输出目录
...
def generate_attack_sequences(gml_path, graph_idx=0):
    ...
    for m in METHODS:
        if m == 'RF':
            ...
            rob, sol = dqn.HXA(G, m)
            sequences[m] = sol
            # print(f"    {m:6s}: 序列长度={len(sol)}, 鲁棒性={rob:.4f}")
...
def main():
    ...
    gml_files = [os.path.join(ROOT_GML_DIR, f)
                 ...
                 if f.startswith('g_') and not f.endswith('.gml')]
    gml_files = sorted(gml_files, key=lambda x: int(os.path.basename(x).split('_')[1].split('(')[0]))
